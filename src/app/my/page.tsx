...
export default async function ClientDashboard() {
  ...
  const [subsResult, enrollResult, profileResult] = await Promise.all([
    supabase
      .from("subscriptions")
    ...
    <div className="space-y-8">
      ...
      <div>
        <h1 className="text-2xl sm:text-3xl font-semibold text-[#2D2327] tracking-tight">
          {t.greeting}, {profile?.full_name?.split(" ")[0] || "there"} 👋
        </h1>
        <p className="text-sm sm:text-base text-[#9B8A8F] mt-1">{t.overview}</p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-[#F0E8EB] p-5 sm:p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold text-[#2D2327]">{t.subscription}</span>
          </div>
        ...
        {activeSub ? (
          <>
            <div className="flex items-end justify-between mb-3">
              <div>
                ...
                <span className="text-sm text-[#9B8A8F] ml-1.5">/ {totalLessons} {t.lessonsRemaining}</span>
              </div>
          ...
          <div className="text-center py-8">
            ...
            <p className="text-sm font-medium text-[#2D2327]">{t.noSubscription}</p>
            <p className="text-xs text-[#9B8A8F] mt-1">{t.noSubscriptionDesc}</p>
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
        ...
        >
          ...
          <div>
            <span className="text-sm font-semibold text-[#2D2327] block">{t.schedule}</span>
            <span className="text-xs text-[#9B8A8F]">{t.browseClasses}</span>
          </div>
        </Link>
        <Link
          href="/my/children"
        ...
        >
          ...
          <div>
            <span className="text-sm font-semibold text-[#2D2327] block">{t.children}</span>
            <span className="text-xs text-[#9B8A8F]">{t.manageProfiles}</span>
          </div>
        </Link>
        <Link
        ...
        >
          ...
          <div>
            <span className="text-sm font-semibold text-[#2D2327] block">{t.subscriptions}</span>
            <span className="text-xs text-[#9B8A8F]">{t.viewPackages}</span>
          </div>
        </Link>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base sm:text-lg font-semibold text-[#2D2327]">{t.upcomingClasses}</h2>
        ...
        {enrollments && enrollments.length > 0 ? (
          ...
          <div className="text-center py-12 bg-white rounded-2xl border border-[#F0E8EB]">
            ...
            <p className="text-sm font-medium text-[#2D2327]">{t.noUpcoming}</p>
            <p className="text-xs text-[#9B8A8F] mt-1 mb-3">{t.noUpcomingDesc}</p>
