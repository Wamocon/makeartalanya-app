...
async function getInstructorPhoto(): Promise<string | null> {
  ...
  try {
    ...
    const { data } = await supabase.storage.from("instructor").list("", { limit: 1 });
    if (!data || data.length === 0) return null;
    const file = data.find((f) => f.name !== ".emptyFolderPlaceholder");
    if (!file) return null;
    return supabase.storage.from("instructor").getPublicUrl(file.name).data.publicUrl;
  } catch {
    return null;
  }
}
...
export default async function AboutSection({ t }: AboutProps) {
  const photoUrl = await getInstructorPhoto();
